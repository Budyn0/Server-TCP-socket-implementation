...
# define PORT 2525          // port_client
# define PORT_SERVER 3525   // port_server
# define MAX_CONNECTIONS 10 // max_połączeń
...
Email mailBox[MAX_CONNECTIONS]
// Przechowywanie tablicy z wiadomościami e-maill
int mailCount = 0
// zmienna przechowująca ilośc maili w tablicy
...
int userCount = 0
// przechowanie ilości użytkowników
...
void * handle_client_connection(void * args)
{
    ...
    // testowanie czy jest socket w non-block
    if (fcntl(fd, F_GETFL) & O_NONBLOCK)
    {
        // socket jest non-block
    }

    // ustawienie socketu w non-block
    ...
    while (1)
    {
             ...
             i++)
        {
            if (strstr(mailBox[i].to, user.username) != NULL)
            {
                ...
                for (int j=i
                     j < mailCount - 1
                     j++) // przesunięcie o 1 indeks w lewo w talbicy po wysłaniu
                {
...
void * handle_server_recv_connection(void * args)
{
    ...
    {
        printf(
            "[server-recv] error while sending mail address.. closing connection\n")
        close(fd)
        pthread_exit(NULL)
    }
...
void * handle_server_send_connection(void * args)
{
    ...
    {
        printf(
            "[server-send] error while receiving the remote mail address.. closing connection\n")
        close(fd)
        pthread_exit(NULL)
    }
...
int create_listen_socket(int port, int max_connections)
{
    ...
    {
        // ustawienie socketu w non-blocking
    ...
    printf(
        "[create_listen_socket, port: %d] Listening started on port %d\n", port, port)
...
int create_server_connection(char * ip, int port)
{
    ...
    {
        printf(
            "[create_server_connection] connected to the server.. %s:%d \n", ip, port)
    }
    ...
    while (1)
    {
        ...
        * /
        ...
        if (remote_server_connfd <= 0)
        {
            ...
            remote_server_connfd = create_server_connection(
                remote_server_ip, PORT_SERVER);
            if (remote_server_connfd > 0)
            {
                ...
                {
                    printf(
                        "Error creating client connection thread: error code %d\n", ret);
